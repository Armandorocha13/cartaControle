...
async function run() {
    console.log("Checking columns for historico_aniel_sp...");
    const { data, error } = await supabase.from('historico_aniel_sp').select('*').limit(1);
    if (error) { console.error(error); return; }
    if (data && data.length > 0) {
        console.log("Columns:", Object.keys(data[0]));
    } else {
        console.log("No data found in historico_aniel_sp");
    }
}
