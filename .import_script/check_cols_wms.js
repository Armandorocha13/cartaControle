...
async function run() {
    console.log("Checking columns for consolidado_wms...");
    const { data, error } = await supabase.from('consolidado_wms').select('*').limit(1);
    if (error) { console.error(error); return; }
    if (data && data.length > 0) {
        console.log("Columns:", Object.keys(data[0]));
    } else {
        console.log("No data found in consolidado_wms");
    }
}
